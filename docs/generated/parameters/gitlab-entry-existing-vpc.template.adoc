...
(`ProvisionBastionHost`)|`Disabled`|Choose "Enabled" to provision a bastion host to access EKS cluster instances.|Instance type
(`BastionInstanceType`)|`t3.micro`|Instance type for the bastion host.
|===
...
(`ConfigureRunner`)|`No`|Choose "Yes" to enable deployment of test GitLab Runner inside EKS cluster.|GetLab Runner Helm chart name
...
(`MaximumConcurrentJobs`)|`10`|The maximum number of concurrent jobs.|Use proviliged mode
(`PriviligedMode`)|`No`|Choose "Yes" to run all containers with the privileged flag enabled. This will allow the docker:dind image to run if you need to run Docker.
|===
